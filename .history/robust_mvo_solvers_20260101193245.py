...
if __name__ == "__main__":
    # 1. Dummy Data
    N = 4
    mu_hat = np.array([0.10, 0.12, 0.15, 0.07])
    
    # Mean uncertainty: +/- 2% return
    delta = np.array([0.02, 0.02, 0.02, 0.02])
    
    # Nominal Covariance
    np.random.seed(42)
    tmp = np.random.randn(N, N)
    Sigma_hat = tmp.T @ tmp + np.eye(N)*0.05
    
    # Covariance uncertainty: +/- 10% elementwise
    box_width = 0.10
    Sigma_lower = Sigma_hat * (1 - box_width)
    Sigma_upper = Sigma_hat * (1 + box_width)
    
    # Ensure bounds are symmetric
    Sigma_lower = (Sigma_lower + Sigma_lower.T)/2
    Sigma_upper = (Sigma_upper + Sigma_upper.T)/2

    # 2. Solve
    # High lambda (risk averse) -> Should pick safer assets/diversify more
    weights = robust_mvo_box_mean_cov(mu_hat, delta, Sigma_lower, Sigma_upper, lam=5.0)

    print("Robust Weights:", np.round(weights, 3))