...
#     Problem Formulation:
#     --------------------
#     Minimize:   Tr(Sigma_hat @ (W + Z)) + epsilon * || P_shape @ (W + Z) @ P_shape ||_F 
#                 - lambda * (w.T @ mu_hat - kappa * || S_mu^(1/2) @ w ||_2)
#     Subject to: [ W    w ]
#                 [ w.T  1 ] >> 0
#                 sum(w) == 1
#                 w >= 0
...
def robust_mvo_general_ellipsoid(mu_hat, Sigma_hat, lam, epsilon, kappa, S_mu, P_shape):
    ...
    prob = cp.Problem(obj, constraints)
    try:
        # Clarabel is generally more robust for SDPs than default SCS
        prob.solve(solver=cp.CLARABEL, verbose=False)
    except:
        try:
            # Fallback to SCS with looser tolerance if Clarabel fails
...
#     Problem Formulation:
#     --------------------
#     Minimize:   w.T @ Sigma_hat @ w + epsilon * (Robust Penalty) 
#                 - lambda * (w.T @ mu_hat - kappa * || S_mu^(1/2) @ w ||_2)
#     Subject to: sum(w) == 1
#                 w >= 0
...
#         w.T @ Q_cov @ w (or sum of squares if Identity).
#     S_mu : (N, N) array
#         Covariance of the mean estimates (typically Sigma / T).
