...
def robust_mvo_general(mu_hat, Sigma_hat, lam, epsilon, kappa, S_mu, P_shape):
    """
    ...
    constraints = [
        ...
        Z >> 0,
        cp.bmat([
            [W, cp.reshape(w, (N, 1), order='C')],  # <--- Fix applied here
            [cp.reshape(w, (1, N), order='C'), np.array([[1.0]])] # <--- And here
        ]) >> 0
    ]
...
def robust_mvo_fast(mu_hat, Sigma_hat, lam, epsilon, kappa, Q_cov, S_mu):
    """
