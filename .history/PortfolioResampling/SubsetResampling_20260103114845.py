...
def naive_mvo_constrained(mu, Sigma, lam):
    """
...
def subset_resampling_mvo(returns, s, b, lam):
    ...
    # ALGORITHM 1: Subset Resampling Loop [cite: 702]
    for j in range(s):
        
        # 1. Randomly sample 'b' indices (without replacement) [cite: 713]
        subset_indices = np.random.choice(N, size=b, replace=False)
        
        # 2. Select the associated return data for this subset [cite: 714]
        subset_returns = returns[:, subset_indices] # Shape (T, b)
        
        # 3. Compute sample Mean and Covariance for the subset 
        mu_sub = np.mean(subset_returns, axis=0)
        Sigma_sub = np.cov(subset_returns, rowvar=False)
        
        # 4. Compute optimal weights for the subset 
        # We use the provided "regular style" MVO logic here.
        w_sub = naive_mvo_constrained(mu_sub, Sigma_sub, lam)
        
        # 5. Construct weights for the whole portfolio [cite: 716]
        # Map the subset weights back to the full N-sized vector.
    ...
    # 6. Aggregate: Average the weights over all 's' resamples [cite: 720]
    avg_weights = cumulative_weights / s
    
    # Final cleanup to ensure weights sum strictly to 1 due to floating point drift
