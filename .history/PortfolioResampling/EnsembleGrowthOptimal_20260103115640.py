...
def ensemble_growth_optimal_portfolio(returns, n1, n2, n3, n4):
    ...
    # Step 6: Compute sample parameters from history
    mu_hat = np.mean(returns, axis=0)
    Sigma_hat = np.cov(returns, rowvar=False)
    
    # Initialize aggregator for the final weights (Step 13)
    # We will sum up the basis portfolios from each 'h' loop here
    final_weight_accumulator = np.zeros(n)
    
    # Step 7: Outer Loop (Bagging / Parametric Bootstrap)
    for h in range(n1):
        
        # Step 8: Generate synthetic data (Parametric Bootstrap)
        ...
        try:
            ...
            # Fallback if Sigma is singular (common in high-dim, low-sample)
            # Add slight jitter to diagonal to fix conditioning
        ...
        # Initialize aggregator for this specific history's basis portfolio (Step 12)
        basis_portfolio_accumulator = np.zeros(n)
        
        # Step 9: Inner Loop (Random Subspace)
        for j in range(n3):
            
            # Step 10: Randomly sample subset of indices
            subset_indices = np.random.choice(n, size=n4, replace=False)
            
            # Step 10b: Select synthetic data for these indices
            subset_data = synthetic_returns[:, subset_indices]
            
            # Step 11: Compute optimal subset weights (Optimization 2)
            # We solve Growth Optimal on the SYNTHETIC data
            w_subset = naive_growth_optimal_constrained(subset_data)
            
            # Step 11b: Construct weights for basis portfolio
